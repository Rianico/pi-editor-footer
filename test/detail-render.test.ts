...
describe("renderDetail", () => {
  ...
  test("returns [] for an empty description", () => {
    assert.deepEqual(renderDetail({ ...item, description: "" }, 40, 5, 0), []);
  });

  test("returns [] for a whitespace-only description", () => {
    assert.deepEqual(renderDetail({ ...item, description: "   \n\t " }, 40, 5, 0), []);
  });
  ...
  test("shrink-to-fit: short description yields 1 header + content lines", () => {
    ...
    assert.deepEqual(lines, ["to-spec · skill", "Turn the current conversation into a spec."]);
  ...
  test("preserves explicit paragraph breaks in the description", () => {
    ...
    assert.deepEqual(lines, ["to-spec · skill", "First paragraph.", "", "Second paragraph."]);
  });
  ...
  test("mid-scroll: only the bottom ellipsis, first visible line stays content", () => {
    const longItem: DetailItem = {
      ...item,
      description: "x".repeat(200),
    };
    const mid = renderDetail(longItem, 20, 5, 3);
    assert.notEqual(mid[1], "..."); // no top ellipsis
    assert.equal(mid[4], "..."); // content still remains below
  });
