...
function renderBarChart(
  ...
  for (let row = chartHeight; row >= 1; row -= 1) {
    ...
    const body = values
      .map((v) =>
        v >= threshold ? theme.fg("accent", glyphs.full) : theme.fg("dim", glyphs.empty),
      )
      .join("");
    lines.push(theme.fg("muted", label) + body);
  }
  lines.push(theme.fg("muted", `  0│${theme.fg("dim", glyphs.axis.repeat(values.length))}`));
  return lines;
}
  ...
  for (let row = chartHeight; row >= 1; row -= 1) {
    ...
    const body = seriesInput
      .map((_, i) => {
        ...
        if (read >= threshold) return theme.fg("accent", glyphs.cacheRead);
        if (write >= threshold) return theme.fg("warning", glyphs.cacheWrite);
        if (inp >= threshold) return theme.fg("muted", glyphs.input);
        return theme.fg("dim", glyphs.empty);
      })
      .join("");
    lines.push(theme.fg("muted", label) + body);
  }
  lines.push(theme.fg("muted", `    0│${theme.fg("dim", glyphs.axis.repeat(seriesInput.length))}`));
...
function renderPerTurnPercent(
  ...
  lines.push(...renderBarChart(theme, glyphs, values, chartHeight));
...
function renderCumulativePercent(
  ...
  lines.push(...renderBarChart(theme, glyphs, bucketed, chartHeight));
...
function renderCumulativeTotal(
  ...
  // Legend
  lines.push(
    theme.fg("muted", glyphs.input) +
      theme.fg("dim", " input (uncached)   ") +
      theme.fg("warning", glyphs.cacheWrite) +
      theme.fg("dim", " cache write   ") +
      theme.fg("accent", glyphs.cacheRead) +
...
export function renderGraphBody(
  ...
  glyphs: ChartGlyphs = UNICODE_CHART_GLYPHS,
): string[] {
  ...
  switch (view) {
    ...
    case "cumulative-total":
      lines.push(
        theme.fg(
          "dim",
          `Aggregate (cumulative) token volumes: input  ${glyphs.input} cacheWrite  ${glyphs.cacheWrite} cacheRead  ${glyphs.cacheRead}`,
        ),
      );
      break;
  }
  ...
  const cumSeries = view !== "per-turn" ? computeCumulativeSeries(messages) : null;

  switch (view) {
    case "per-turn":
      lines.push(...renderPerTurnPercent(theme, glyphs, messages, chartWidth));
      break;
    case "cumulative-percent":
      lines.push(...renderCumulativePercent(theme, glyphs, messages, chartWidth, cumSeries!));
      break;
    case "cumulative-total":
      lines.push(...renderCumulativeTotal(theme, glyphs, messages, chartWidth, cumSeries!));
      break;
  }
