...
### Context window section color — hex tiers, not theme tokens

- `contextUsageColor(pct)` (4 semantic tiers, no window awareness) replaced by `color-policy.ts:contextUsageTier(pct, contextWindow)` + `CONTEXT_TIER_HEX` (`#A3BE8C` / `#EBCB8B` / `#9A3939`). **1M+ windows** escalate at **12.5 % / 25 %**; **smaller windows** at **25 % / 50 %**. `chrome-state.ts:contextPainter` colors the whole `pct · tokens/window` section (and the icon-bar fill) with one tier color — the cache `c` section keeps `cacheHitColor` and is untouched.
- **Trap: `theme.fg("#A3BE8C", s)` throws `Unknown theme color`** — the pi theme resolves *named tokens only* (`getFgAnsi` throws too), so an exact hex cannot route through it. Emit raw SGR: `ansi-color.ts:hexFg(hex, mode)` with `mode = theme.getColorMode()` (`"truecolor"`; anything else quantizes to the closest xterm-256 index). `chrome-theme.ts:adaptTheme` (formerly the `ChromeComposition` class) exposes it as `fgHex`; `Theme.fgHex?` stays **optional**, so a theme without a color mode degrades to `CONTEXT_TIER_THEME_COLOR` (`success` / `warning` / `error`) instead of crashing.
- `ansi-color.ts` is the single source of the RGB/256 quantizer: `model-info.ts`'s private copies (BASIC16/CUBE/GRAY/`rgbTo256`/`parseFgAnsiToRgb` + a local `stripAnsi`) were deleted and now import from it.
...
- Update: the `timeline.wallTime`/`tokens`/`cost` toggles are now real in `transcript-timeline.ts:buildTimelineText` (they were no-ops with identical if/else branches) — each off omits its segment (`tokens` also drops the cache `c x%`, a token metric with no config row of its own); `dt` and line2 are never gated.
...
### Timeline interleaved in chat container, not stacked aboveEditor

- `aboveEditor` widget (`setWidget("wall-time", …, {placement:"aboveEditor"})` with `wallTimeHistory[]` stacked) renders fixed above input (`2026-08-21 …` stacked above `↑·↓|T1` + model border) — does not scroll with transcript, duplicates on rebuild, and is what user reported as weird stacked view. `pi-tui v0.84.2` only exposes `aboveEditor|belowEditor` for `setWidget`, so stacked is the only widget option.
- Fix: use public `pi.appendEntry("timeline", {text})` + `pi.registerEntryRenderer("timeline", (entry,_,theme)=> new Text(dim))` — `type:"custom"` entries are rendered in `chatContainer` transcript order, interleaved `User/Assistant/<dim>` (`2026-08-21 …` / `1 turns…` dim left-aligned), scroll with history, and never sent to LLM (`custom` not in `context`). `src/index.ts` now does this at `agent_settled` via `extensionPi` global (set at `export default` entry) and registers renderer at startup; `src/transcript-timeline.ts` direct `chatContainer.addChild`/`InteractiveMode.prototype` patch (global scan, async `import` race, sync `require` ESM failure) is brittle and deprecated — kept only for `void timeline` compat, not used for rendering.

### Footer `↑` counted only uncached input — now total input tokens

- pi-ai `Usage.input` is the *uncached* prompt only — the cached prefix lives in `cacheRead`/`cacheWrite`, and `totalTokens = input + output + cacheRead + cacheWrite` (`pi-ai` `calculateCost` already uses `input + cacheRead + cacheWrite` as the billed input for cost tiers). `src/footer.ts` pushed `fmtTokens(totals.input)`, so a cached session read `↑ 395 · ↓ 505 · $0.00` next to a `416k/1.0M` context bar.
- Fix: `src/state.ts:totalInputTokens()` delegates to the single source `src/cache-math.ts:promptTokens()` (`input + cacheRead + cacheWrite`); the footer's right block is `↑ <total input> · ↓ <output> · $<cost>`. Cache stays out of the footer as its own segment — the top context bar's `c <hit%>` remains the only cache readout.
- `getUsageTotals` now also sums `toolResult` message usage and `branch_summary`/`compaction` entry usage (parity with pi core `addUsageToTotals`) — nested model calls are billed, so they belong in session totals. It also takes `UsageTotalsSource` instead of the two inline structural casts, which deleted ~35 duplicated lines from `installFooter`.
- Scope boundary (explicit, do not "fix"): **Agent run** accounting stays on raw `usage.input` — `TurnTelemetryTracker`'s per-turn `inputTokens`, `AgentRunLedger`'s max-vs-sum + context-window caps, and therefore the top-border/timeline `↑` answer *how much this agent run moved*, not *what the session billed*. They are intentionally NOT folded into `totalInputTokens`, so a per-run `↑ 4k` beside a footer `↑ 352k` is expected — two different questions, not a bug.

## Verification — how to check the style

- **TUI smoke**: `pi --no-session -nc -ne -ns -nt -nbt -e src/index.ts` — top shows `provider/model · thinking | # [#####-------] 39.6% · 416k/1.0M | c 0.0%` left (or `| c 85.3%` with cache) and `T1 · 8s · 2 tools` right; bottom shows `> TPS 60.6 tok/s | ~ TTFT 2.5s | + 8.3s | ↑ 395 | ↓ 505 | $0.16` right. Footer shows `cwd · git • runtime` left / `↑ 352k · ↓ 505 · $0.00` right — `↑` is *total* input (uncached + cache read + cache write, `totalInputTokens`), so it tracks the context bar's `416k` rather than `usage.input` alone; cache is still never a footer segment (top bar's `c 85.3%` only), no `#` in center.
