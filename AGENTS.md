...
- Known sibling undercount, deliberately not fixed here: `TurnTelemetryTracker` still sums `usage.input` for per-turn `inputTokens`, so the top-border/timeline per-agent `↑` excludes cache.
