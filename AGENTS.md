...
- Fix: `src/state.ts:totalInputTokens()` is the single source (`input + cacheRead + cacheWrite`); the footer's right block is `↑ <total input> · ↓ <output> · $<cost>`. Cache stays out of the footer as its own segment — the top context bar's `c <hit%>` remains the only cache readout.
