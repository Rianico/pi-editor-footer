...
- **Trap: `theme.fg("#A3BE8C", s)` throws `Unknown theme color`** — the pi theme resolves *named tokens only* (`getFgAnsi` throws too), so an exact hex cannot route through it. Emit raw SGR: `ansi-color.ts:hexFg(hex, mode)` with `mode = theme.getColorMode()` (`"truecolor"`; anything else quantizes to the closest xterm-256 index). `ChromeComposition.adaptTheme` exposes it as `fgHex`; `Theme.fgHex?` stays **optional**, so a theme without a color mode degrades to `CONTEXT_TIER_THEME_COLOR` (`success` / `warning` / `error`) instead of crashing.
